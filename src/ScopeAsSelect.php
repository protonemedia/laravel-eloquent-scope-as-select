...
class ScopeAsSelect
{
    ...
    public static function addMacro(string $name)
    {
        Builder::macro($name, function (string $name, callable $callable) {
            $query = ScopeAsSelect::builder($this);

            $originalTable = $query->getModel()->getTable();

            $aliasedTable = "{$name}_{$originalTable}";
            $aliasedModel = $query->getModel()->newModelInstance()->setTable($aliasedTable);
